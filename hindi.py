...
st.title("Welcome to Hindi Movies")
...
for i in id_list:
    ...
    if lang_list[indi] == "Hindi":
        hindi.append(title_list[indi])

r= math.ceil(len(hindi)/3)
k= -1
for l in range(r):
    col= st.columns(3)
    for cols in range(3):
        with col[cols]:
            ...
            if k>len(hindi)-1:
                break
            if st.button(hindi[k]):
                ind = title_list.index(hindi[k])
